...
-(NSMutableDictionary *)sortByDepartment
{
    NSMutableDictionary *dic = [NSMutableDictionary new];
    for (MTTUserEntity * user in [[DDUserModule shareInstance] getAllMaintanceUser]) {
        MTTDepartmentEntity *e = [[DDDepartmentModule instance] getDepartmentByGId:user.department];
        if(!e){
            continue;
        }
        NSString *department = e.dept_name;
        
        
        if ([dic safeObjectForKey:department]) {
            NSMutableArray *arr = [dic safeObjectForKey:department];
            [arr addObject:user];
        }else{
            NSMutableArray *arr = [[NSMutableArray alloc] initWithArray:@[user]];
            [dic safeSetObject:arr forKey:department];
        }
    }
    [dic enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        NSArray *preData = obj;
