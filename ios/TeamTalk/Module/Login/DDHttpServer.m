...
-(void)getMsgIp:(void(^)(NSDictionary *dic))block failure:(void(^)(NSString* error))failure
{
    ...
    [manager GET:[MTTUtil getServer] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
-(void)getDiscovery:(void(^)(NSArray *dic))block failure:(void(^)(NSString* error))failure{
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    [manager GET:[LoginModule instance].discovery parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
