...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        else
        {
            ...
            uint32_t userCnt =rsp.userIdListArray_Count;
            group = [[MTTGroupEntity alloc] init];
            group.objID = groupId;
            group.name = groupName;
            group.groupType = GROUP_TYPE_TEMPORARY;
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint16_t seqNo)
    {
        ...
        [req.memberIdListArray removeAll];
        for(NSNumber * num in originalID){
            [req.memberIdListArray addValue:num.intValue];
        }
