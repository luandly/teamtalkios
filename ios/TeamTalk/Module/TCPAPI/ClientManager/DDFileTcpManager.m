...
@interface DDFileTcpManager(PrivateAPI)
...
@implementation DDFileTcpManager
+ (instancetype)instance
{
    static DDFileTcpManager* g_tcpClientManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        g_tcpClientManager = [[DDFileTcpManager alloc] init];
    });
    ...
    if (aStream) {
        ...
        if (len > 0) {
            ...
            while ([_receiveBuffer length] >= 16) {
                ...
                NSLog(@"***********收到文件服务端sid:%i cid:%i  seqNO:%i",tcpHeader.serviceId,tcpHeader.commandId,tcpHeader.reserved);
