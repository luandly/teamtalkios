...
{
    if(api.requestCommendID == FileCmdID_CidFilePullDataRsp){
        return YES;
    }
    ...
    dispatch_sync(self.apiScheduleQueue, ^{
        ...
        {
            NSLog(@"registerApi:REQUEST:%@",MAP_REQUEST_KEY(api));
        ...
        {
            NSLog(@"registerApi:RESPONSE:%@",MAP_RESPONSE_KEY(api));
...
- (void)sendData:(NSMutableData*)data isFileServer:(BOOL)isFile
{
    dispatch_async(self.apiScheduleQueue, ^{
        if(isFile){
            [[DDFileTcpManager instance] writeToSocket:data];
        }else{
