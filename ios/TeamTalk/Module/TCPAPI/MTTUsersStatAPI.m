...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        for (UserStat *userStat in allUsersStatRsp.userStatListArray) {
...
- (Package)packageRequestObject
{
    ...
    {
        IMUsersStatReq *queryPush = [[IMUsersStatReq alloc] init];
        NSArray* array = (NSArray*)object;
        [queryPush setUserId:0];
        [queryPush.userIdListArray removeAll];
        for(NSNumber *num in array){
            int n = num.intValue;
            [queryPush.userIdListArray addValue:n];
        }
