...
- (void)deleteRecentGroup:(MTTGroupEntity *)group completion:(InsertsRecentContactsCOmplection)completion
{
    [_dataBaseQueue inDatabase:^(FMDatabase *db) {
        [_database beginTransaction];
        __block BOOL isRollBack = NO;
        @try {
            ...
            BOOL result = [_database executeUpdate:sql,group.objID];
            if (!result)
            {
                isRollBack = YES;
            }
        ...
        @finally {
            if (isRollBack)
            {
                [_database rollback];
                DDLog(@"insert to database failure content");
                NSError* error = [NSError errorWithDomain:@"删除最近群失败" code:0 userInfo:nil];
                completion(error);
            }
            else
            {
