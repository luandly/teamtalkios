...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(pcLoginNotification:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(groupChange:)
                                                 name:DDNotificationLoadLocalGroupChange
                                               object:nil];
...
-(void)pcLoginNotification:(NSNotification*)notification
{
    ...
    if([[dic objectForKey: @"loginStat"] intValue]== UserStatType_UserStatusOffline){
        self.isMacOnline = 0;
    }else{
        self.isMacOnline = 1;
    }
...
-(void)groupChange:(NSNotification *)notification{
    MTTGroupEntity *group = [notification object];
    if(!group){
        return;
    }
    
    if(![group isKindOfClass:[MTTGroupEntity class]]){
        return;
    }
    ...
    switch (group.groupType) {
        case GroupModifyType_GroupModifyTypeAdd:
            {
                [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:[NSString stringWithFormat:@"有人加入了群:%@",group.name]];
            }
            break;
        case GroupModifyType_GroupModifyTypeDel:
        {
            [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:[NSString stringWithFormat:@"有人从%@群里被删除了",group.name]];
        }
            break;
        case GroupModifyType_GroupModifyTypeExit:
        {
            [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:[NSString stringWithFormat:@"有人退出了群:%@",group.name]];
        }
            break;
        case GroupModifyType_GroupModifyTypeDismiss:
        {
            [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:[NSString stringWithFormat:@"临时群%@解散了",group.name]];
        }
            break;
        default:
            break;
