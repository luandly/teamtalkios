...
- (void)viewDidLoad
{
    ...
    if(self.group.groupType == GROUP_TYPE_TEMPORARY){
        
        [self.view addSubview:self.quit];
        self.quit.userInteractionEnabled = YES;
        self.quit.enabled = YES;
        self.quit.hidden = NO;
        [self.quit setClipsToBounds:YES];
        [self.quit.layer setCornerRadius:5];
        if ([self.group.groupCreatorId isEqual:TheRuntime.user.objID]){
            [self.quit setTitle:@"解散" forState:UIControlStateNormal];
            
        }else{
            [self.quit setTitle:@"退出" forState:UIControlStateNormal];
        }
        [self.quit setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [self.quit setBackgroundColor:TTBLUE];
        int count = (int)(self.items.count / 3);
        int top = 360 + count * 60 < SCREEN_HEIGHT - 40 ? 360 + count * 60:SCREEN_HEIGHT - 40;
        ...
        [self.quit addTarget:self action:@selector(clickquit) forControlEvents:UIControlEventTouchUpInside];
    }
    else{
...
-(void)quitgroup{
    ...
    DDDeleteMemberFromGroupAPI* deleteMemberAPI = [[DDDeleteMemberFromGroupAPI alloc] init];
    deleteMemberAPI.isgroup = YES;
    if ([self.group.groupCreatorId isEqual:TheRuntime.user.objID]){
        
        deleteMemberAPI.type = GroupModifyType_GroupModifyTypeDismiss;
        
    }else{
        deleteMemberAPI.type = GroupModifyType_GroupModifyTypeExit;
    }
    [deleteMemberAPI requestWithObject:@[self.session.sessionID, TheRuntime.user.objID] Completion:^(MTTGroupEntity *response, NSError *error) {
        [self.hud hide:YES afterDelay:1];
        if (error) {
            [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:@"未知错误"];
            return ;
        ...
        if (response)
        {
            ...
            [[MTTDatabaseUtil instance] deleteRecentGroup:self.group completion:^(NSError *error) {
                
            }];
            [[SessionModule instance] removeSessionByServer:self.session];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ReloginSuccess" object:nil];
            if ([self.group.groupCreatorId isEqual:TheRuntime.user.objID]){
                [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:@"您已成功解散该群"];
            }else{
                [MTTUtil showAlertWithTitle:@" " message:error.domain?error.domain:@"您已成功退出该群"];
            }
