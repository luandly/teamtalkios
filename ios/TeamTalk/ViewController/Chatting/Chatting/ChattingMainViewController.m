...
- (UITableViewCell*)p_fileCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(MTTMessageEntity*)message;
...
-(void)sendFileMessage:(NSString *)text{
    DDMessageContentType msgContentType = DDMessageTypeText;
    MTTMessageEntity *message = [MTTMessageEntity makeMessage:text Module:self.module MsgType:msgContentType];
    message.msgType = MsgType_MsgTypeOfflineFile;
    [self.tableView reloadData];
    [[MTTDatabaseUtil instance] insertMessages:@[message] success:^{
        DDLog(@"消息插入DB成功");
    } failure:^(NSString *errorDescripe) {
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[MTTMessageEntity class]])
    {
        ...
        if (message.msgContentType == DDMessageTypeText ) {
            if(message.msgType == MsgType_MsgTypeOfflineFile){
                cell = [self p_fileCell_tableView:tableView cellForRowAtIndexPath:indexPath message:message];
            }else{
...
- (UITableViewCell*)p_fileCell_tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath message:(MTTMessageEntity*)message
{
    ...
    if (!cell)
    {
        cell = [[DDChatFileCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        cell.contentLabel.delegate = self;
    }
    cell.session =self.module.MTTSessionEntity;
    NSString* myUserID = [RuntimeStatus instance].user.objID;
