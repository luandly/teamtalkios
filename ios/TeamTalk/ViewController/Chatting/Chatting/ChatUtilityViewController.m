...
- (void)viewDidLoad
{
    ...
    UIButton *choosefileBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.view addSubview:choosefileBtn];
    [choosefileBtn setBackgroundImage:[UIImage imageNamed:@"chat_pick_file_plus"] forState:UIControlStateNormal];
    [choosefileBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(middleView1);
        make.top.mas_equalTo(15);
        make.size.mas_equalTo(CGSizeMake(65, 65));
    }];
    [choosefileBtn setClipsToBounds:YES];
    [choosefileBtn.layer setCornerRadius:5];
    [choosefileBtn.layer setBorderWidth:0.5];
    [choosefileBtn.layer setBorderColor:RGB(174, 177, 181).CGColor];
    [choosefileBtn addTarget:self action:@selector(chooseFile:) forControlEvents:UIControlEventTouchUpInside];
    
    UILabel *chooseFileLabel = [UILabel new];
    [chooseFileLabel setText:@"文件"];
    [chooseFileLabel setTextAlignment:NSTextAlignmentCenter];
    [chooseFileLabel setFont:systemFont(13)];
    [chooseFileLabel setTextColor:RGB(174, 177, 181)];
    [self.view addSubview:chooseFileLabel];
    [chooseFileLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(middleView1);
        make.top.mas_equalTo(choosefileBtn.mas_bottom).offset(15);
        make.width.equalTo(middleView1);
        make.height.mas_equalTo(13);
    }];
}
...
-(void)chooseFile:(id)sender{
    SelectFileViewController *select = [[SelectFileViewController alloc] init];
    select.userId = self.userId;
    [self pushViewController:select animated:YES];
}
