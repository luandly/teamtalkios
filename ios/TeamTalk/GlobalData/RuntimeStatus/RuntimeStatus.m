...
-(void)registerAPI
{
    ...
    //用户信息变化
    MTTBuddyUserinfoNotify *b = [MTTBuddyUserinfoNotify new];
    [b registerAPIInAPIScheduleReceiveData:^(id object, NSError *error) {
    }];
    
    //群组信息变化
    MTTBuddyDepartmentNotify *d = [MTTBuddyDepartmentNotify new];
    [d registerAPIInAPIScheduleReceiveData:^(id object, NSError *error) {
    }];
    
    //退出群通知
    MTTGroupQuitNotify *g = [MTTGroupQuitNotify new];
    [g registerAPIInAPIScheduleReceiveData:^(id object, NSError *error) {
        if(object){
            [[NSNotificationCenter defaultCenter] postNotificationName:DDNotificationLoadLocalGroupChange object:object];
        }
    }];
    
    //传输文件通知
    MTTFilePullNotify *f = [MTTFilePullNotify new];
    [f registerAPIInAPIScheduleReceiveData:^(id object, NSError *error) {
        if(object){
            [[NSNotificationCenter defaultCenter] postNotificationName:DDNotificationFilePull object:object];
        }
    }];
    
    //传输文件状态通知
    MTTFileStateNotify *s = [MTTFileStateNotify new];
    [s registerAPIInAPIScheduleReceiveData:^(id object, NSError *error) {
